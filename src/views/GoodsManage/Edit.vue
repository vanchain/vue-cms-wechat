		...
		<div class="text">
			<el-form :rules="rules" ref="form" :model="form" label-width="100px">
				...
				<el-form-item label="商品轮播图" prop="slider">
					<el-upload :file-list="fileList" :headers="headers" action="/api/upload/slider" list-type="picture-card" :on-success="uploadSuccess"
...
<script>
	...
	export default {
		...
		async mounted() {
			// 获取商品详情
			let goods = await this.loadGoodsInfo(this.id);
			this.form = goods;
		...
		methods: {
			async loadGoodsInfo(id) {
				let { status, data } = await Goods.goodsInfo({ id });
				if (status) {
					// this.form = data;
					this.sliderString = data.slider;
					return data;
				}
			},
