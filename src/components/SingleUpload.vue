<template>
	<el-upload :headers="headers" class="avatar-uploader" :action="'http://localhost:3003'+uploadAddress" :show-file-list="false"
	...
	export default {
		...
		methods: {
			async handleRemove() {
				...
				let src = this.url;
				let { status } = await Upload.delete_avatar({ src });
				if (status) {
